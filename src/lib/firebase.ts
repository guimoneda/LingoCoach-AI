...
// Combine env vars with local config
...
const missingKeys = Object.entries(firebaseConfig)
  .filter(([key, value]) => !value && key !== 'firestoreDatabaseId' && key !== 'measurementId')
  .map(([key]) => key);

if (missingKeys.length > 0) {
  console.warn(`Missing Firebase configuration keys: ${missingKeys.join(', ')}. Please check your environment variables.`);
}
